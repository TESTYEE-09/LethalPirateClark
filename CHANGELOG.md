...
### Asset bundle removed — prefab built in pure C# at runtime
The asset-bundle approach that powered v1.0.0 through v2.1.2 was the source of every "loads but does nothing" failure across the v1.x and v2.x line. The root cause was that the bundle's NetworkObject carries a `GlobalObjectIdHash` that has to match exactly between host and client; mismatches silently break replication. To keep that hash in sync, the bundle was built from a Unity editor, and the editor-installed-on-dev-machine had drifted its `PlaybackEngines`/`Library` state enough that the bundle it built could not be loaded back by the same editor, let alone by the live game. Every v1.4.1+ attempt to fix this (a new editor install, a different build target, embedded fallback, fingerprint diagnostics) ran into the same wall: the bundle-load layer itself is unreliable on this game/runtime combo.

v3.0.0 cuts the knot. There is no `stilllife` file in the install. The entire prefab — `GameObject`, `NetworkObject` (with a stable baked-in `GlobalObjectIdHash = 0xC1A12C1B`), `NetworkTransform`, `NavMeshAgent`, body + trigger `CapsuleCollider`s, `Animator`, two `AudioSource`s, a child "Collision" with `EnemyAICollisionDetect`, and the `StillLifeAI` itself — is constructed in C# during BepInEx `Awake`. The `EnemyType` is a fresh `ScriptableObject.CreateInstance<EnemyType>()` with every spawn-critical field reflection-set. The mod is now a single DLL; there is no second file to mismatch, no Unity editor required, no version-skew window.

- **No more `stilllife` asset bundle shipped.** The DLL is the only file in `plugins/.../StillLife/`. The install is one file, like every other BepInEx plugin. If an old `stilllife` file is left over from a previous install it's silently ignored (a warning is logged).
- **`BuildPrefab.BuildProceduralPrefab` (new) builds the GameObject hierarchy.** Adds `NetworkObject` (with a stable hash), `NetworkTransform`, `NavMeshAgent` (radius 0.35 / height 1.9 / speed 3.2), `MeshFilter`+`MeshRenderer` with a primitive capsule + off-white "foam body" Standard material, body `CapsuleCollider` (solid, Y-axis, height 2), `Animator` (no controller, but the `frozen` bool parameter is the only one the AI writes — Unity logs a warning, not an error, when an Animator has no controller), two `AudioSource`s (one 2D ambient, one 3D sfx; no clips — every `PlayOneShot/Play` in the AI code is null-guarded), and a `Collision` child with a trigger `CapsuleCollider` + `EnemyAICollisionDetect`. `StillLifeAI` is NOT added here — the same deferred-attach path the bundle version used (waits for `RoundManager.Instance`) is reused, so `EnemyAI.Awake`'s `RoundManager.Instance` reads don't NRE.
- **`BuildPrefab.BuildProceduralEnemyType` (new) builds the `EnemyType`.** All public EnemyType fields are filled in: `enemyName="Pirate Clark"`, `PowerLevel=1.5`, `MaxCount=<config>`, `isOutsideEnemy=false`, `isDaytimeEnemy=false`, `increasedChanceInterior=1`, `canBeStunned=true`, `canDie=true`, `canBeDestroyed=true`, `destroyOnDeath=true`, `canSeeThroughFog=true`, `disableAnimatorWhenFar=true`, `pushPlayerForce=5`, `pushPlayerDistance=1.5`, `stunTimeMultiplier=1`, `doorSpeedMultiplier=0` (cannot open doors), `stunGameDifficultyMultiplier=1`, default `AnimationCurve`s, empty `audioClips`/`miscAnimations` lists. The prefab reference is wired back via `enemy.enemyPrefab = prefab`.
- **`Plugin.LoadAssetsAndRegister` rewritten.** Now: try the on-disk bundle (in case a user has one from a previous install and wants the editor-built model). On null, try the embedded base64 copy (defence in depth). On null, call `BuildProceduralEnemyAndRegister`. The bundle path is still wired up so a user with a v2.x `stilllife` file lying around gets the model pirate if their machine happens to load it; everyone else gets the procedural capsule. Either way the registration code that follows is identical.
- **`Plugin.PreparePrefab` made null-bundle safe.** The audio-clip loop that used to iterate `_bundle!.LoadAllAssets<AudioClip>()` is now `if (_bundle != null) foreach (...)`, so the procedural path doesn't NRE. Audio is silent in v3.0.0 — the AI's `creatureSFX.PlayOneShot(eatClip)` calls are null-guarded and simply no-op when `eatClip` is null. (The model is a primitive capsule too, not a pirate, so the user sees a beige capsule stalk them — the *behaviour* is identical to the bundle build, only the visuals are placeholders.)
- **Bestiary entry not yet wired in v3.0.0.** The TerminalNode/TerminalKeyword the bundle carried were authored against the stilllife model's voice lines, which we no longer have a procedural source for. `Enemies.RegisterEnemy` is called with null node/keyword — Pirate Clark still spawns and behaves correctly, just doesn't show a "BESTIARY" entry in the ship's terminal. A v3.1.0 with a hand-written entry is the next step.
- **Hard cutover from v2.x.** The `GlobalObjectIdHash` is a new constant (`0xC1A12C1B`). An old v2.x client trying to join a v3.0.0 host will reject the prefab and the host will log a NetworkObject mismatch; this is expected. Everyone in a co-op session needs the same version. (v2.x's hash was whatever Unity derived from the bundle's prefab GUID; we're not recreating that on purpose — a clean constant is more reliable than reverse-engineering a build artifact.)

### Hard cutover rationale
v1.4.1 fixed the `GlobalObjectIdHash` for Unity 2022.3.62, but the bundle-build path was still required to get a hash in the first place. v2.0.0 added co-op RPC support via the NetcodePatcher. v2.1.0 hardened the deferred-attach and collider paths. v2.1.1 added fingerprint diagnostics. v2.1.2 added the embedded-base64 fallback. None of these made the bundle *actually load* on every install — the failure mode (Unity runtime rejecting the bundle with no error) was upstream of any code we could write. v3.0.0 removes the bundle from the equation entirely. The visible change is "Pirate Clark is a beige capsule instead of a furry pirate model" — the gameplay is unchanged. A follow-up release can ship a hand-modelled mesh and a hand-written bestiary entry without re-litigating the bundle problem.
